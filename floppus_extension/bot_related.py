...
from bot_utils.shared_data import (
    CHANNEL__BOT_BUG_REPORTS,
    CHANNEL__BOT_SUGGESTIONS,
    GUILD__NEET_GUILD,
    GUILD__NEKO_DUNGEON,
    INVITE__SUPPORT_GUILD,
    LINK__HATA,
    LINK__PYTHON,
    LINK__RICK_ROLL,
    LINK__SOURCE,
...
from bot_utils.utils import (
    chad_time_delta,
    get_event_color,
    get_files_and_code_lines_count,
)
from hata import CLIENTS, GUILDS, USERS, Client, Embed, __version__, time_to_id
from hata.ext.commands import Closer, Pagination
...
Floppus: Client
BOT_STUFF = Floppus.interactions(
    None, name="Bot", description="Bot Related Slash", is_global=True
)
...
async def info(client, event):
    ...
    pages.append(
        ...
        .add_field(
            "Owner", f"{','.join([user.full_name for user in client.owners])}", True
        )
        .add_field(
            "Support Guild", f"[{GUILD__NEET_GUILD.name}]({support_guild_url})", True
        )
        ...
        .add_field(
            "Slash Count", f"{len(client.slasher.command_id_to_command):,}", True
        )
        .add_thumbnail(icon)
    )
    ...
    pages.append(
        ...
        .add_field(
            "Uptime",
            f"{chad_time_delta((datetime.now() - ALIVE_TIME), '{d}D {h}H {m}M')}",
            True,
        )
        .add_field("Shards", f"{client.shard_count:,}")
        .add_field("Source", f"[Github]({LINK__SOURCE})", True)
        .add_field(
            "Python Ver", f"[Python {version[0]}.{version[1]}]({LINK__PYTHON})", True
        )
        .add_field("Lib Ver", f"[Hata {__version__}]({LINK__HATA})", True)
        .add_field(
            "Memory Usage",
            f"{formatify(virtual_mem.used)}/{formatify(virtual_mem.total)}",
            True,
        )
    ...
    pages.append(
        Embed("Join Us! Click me!", url=support_guild_url).add_image(
            f"https://discordapp.com/api/guilds/{GUILD__NEET_GUILD.id}/widget.png?style=banner4"
        )
    )
...
async def help_(client, event):
    ...
    for slashes in client.slasher.command_id_to_command.values():
        embed = Embed(
            f"{MAIN_COMMANDS_ICON} {slashes.name.capitalize()}",
            f">>> {slashes.description}",
        )

        for slash in slashes._sub_commands.values():
            embed.add_field(
                f"{SUB_COMMANDS_ICON} {slash.name.capitalize()}",
                f">>> {slash.description}",
            )
...
async def invite(
    ...
    await Closer(
        ...
        Embed(
            "Here is the link",
            "I hope to see you in your guild",
            color=get_event_color(event),
            url=choice,
...
async def report_bug(client, event, msg: ("str", "Your Report")):
    ...
    yield await Closer(
        client, event, Embed("Thank you for letting us know !!!", color=color)
    )
