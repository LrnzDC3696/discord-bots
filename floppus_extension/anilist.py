...
from hata import Client, Color, Embed, chunkify
from hata.backend.utils import to_json
from hata.ext.commands import Pagination

Floppus: Client

ANILIST = Floppus.interactions(
    None, name="Anilist", description="Animanga stuff", is_global=True
)
...
async def post_request(anilist_request, type_):
    """Posts the requests"""
    async with Floppus.http.post(
        ANILIST_URL,
        headers={"content-type": "application/json"},
        data=to_json(anilist_request),
    ) as response:
...
async def anime(client, event, title: ("str", "what is the anime title?")):
    ...
    try:
        ...
        yield Embed(
            "Oops...",
            "There seems to be a problem with the api right now try again later!",
        )
        return
    except NotFound:
    ...
    pages.append(
        ...
        .add_image(banner_image)
        .add_thumbnail(cover_img)
    )
    ...
    await Pagination(
        client, event, chadify(pages, get_event_color(event), result["siteUrl"])
    )
...
async def staff(client, event, name: ("str", "What is the staff name?")):
    ...
    for character in characters:
        pages.append(
            Embed(character["name"]["full"], url=character["siteUrl"])
            .add_image(character["image"]["large"])
            .add_author(image, name)
        )
...
async def studio(client, event, name: ("str", "What is the studio name?")):
    ...
    for anime in animes:
        pages.append(
            Embed(anime["title"]["romaji"], url=anime["siteUrl"]).add_image(
